...
def test_search_and_analysis_flow(client):
    ...
    assert len(detail.json()["history"]) == 4
    ...
    history = client.get(
        f"/api/v1/trade/history?hs_code=902620&country_iso3={rows[0]['destination_iso3']}"
    )
    assert len(history.json()) == 4
    suppliers = client.get(
        f"/api/v1/trade/suppliers?hs_code=902620&country_iso3={rows[0]['destination_iso3']}&year=2025"
    )
    assert suppliers.status_code == 200


def test_all_hs_analysis_catalog(client):
    response = client.get("/api/v1/catalog/hs?page=1&page_size=20")
    assert response.status_code == 200
    payload = response.json()
    assert payload["summary"]["total_hs"] > 5_000
    assert len(payload["items"]) == 20
    assert all(len(item["hs_code"]) == 6 for item in payload["items"])

    match = client.get("/api/v1/catalog/hs?q=902620&page_size=10").json()
    assert match["items"][0]["hs_code"] == "902620"
...
def test_idempotent_manual_macro_import(client):
    ...
    assert (
        len([row for row in rows if row["metric_key"] == "gdp_usd" and row["period_year"] == 2025])
        == 1
    )
